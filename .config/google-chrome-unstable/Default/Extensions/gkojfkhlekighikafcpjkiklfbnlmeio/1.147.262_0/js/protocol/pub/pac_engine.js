...
E.init = function(json, options){
    ...
    rules = json.unblocker_plus||{};
    for (i in rules)
    {
        rule = rules[i];
        parse_cmds(i, rule, rules, g_pac_engine.exceptions_plus);
    }
    return 0;
};
...
E.find_proxy_for_url_exception = function(url, host, orig, is_plus){
    var ret = find_proxy_for_url(url, host, is_plus ?
        g_pac_engine.exceptions_plus : g_pac_engine.exceptions, {exception: 1,
        orig_proxy: orig});
