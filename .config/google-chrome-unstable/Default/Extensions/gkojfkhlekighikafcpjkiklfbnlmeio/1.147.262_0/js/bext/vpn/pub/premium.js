...
define(['/bext/pub/locale.js', 'jquery', '/bext/pub/lib.js',
    ...
    '/bext/vpn/pub/dev_mode.js', '/util/version_util.js',
    ...
    version_util, storage, force_lib, info){
...
var membership_timeout, daily_refresh_timer, refresh_user_timeout;
...
E.refresh_user = function(opt){
    ...
    {
        be_lib.perr(zerr.L.NOTICE, {id: 'refresh_user_multiple', rate_limit:
            {count: 2}, info: {user_id: user_id}});
    }
    last_refreshed = now;
    return etask({name: 'refresh_user', cancel: true}, [function(){
        ...
        clearTimeout(refresh_user_timeout);
...
function get_force_rule(conf_name, root_url, opt){
    ...
    if (!opt.ignore_install_version && rule.install_ver_min && install_ver &&
        version_util.cmp(install_ver, rule.install_ver_min)<0)
    {
        return false;
    }
...
E.init = function(be_rule){
    ...
    info.on('inited', function(){
        if (zutil.is_mocha() && last_refreshed)
            return;
...
function on_cookie_change(change){
    ...
    var min_ver = (be_ext.get('bext_config')||{})
        .signin_redirect_back_to_site_min_ver;
    if (!version_util.valid(min_ver)||
        version_util.cmp(be_util.version(), min_ver)<0)
    {
        return;
    }
    refresh_user_timeout = setTimeout(E.refresh_user, 3*date.ms.SEC);
}
