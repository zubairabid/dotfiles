...
define(['jquery', 'underscore', '/bext/pub/backbone.js', '/util/etask.js',
    ...
    '/util/date.js', '/util/util.js', 'backbone',
    '/util/user_agent.js', '/util/storage.js', '/util/util.js',
    '/bext/pub/ga.js', '/util/escape.js', '/bext/vpn/pub/ui_obj.js',
    '/bext/vpn/pub/ajax.js', '/bext/vpn/pub/site_premium_ui.js',
    '/bext/vpn/pub/debug_ui.js', '/bext/vpn/pub/ui_lib.js',
    '/bext/vpn/pub/templates.js', '/bext/vpn/pub/privacy.js'],
    function($, _, be_backbone, etask, be_vpn_util, svc_util, version_util,
    zerr, be_util, T, B, zurl, string, be_popup_lib, pcountries, date,
    util, Backbone, user_agent, storage, zutil, ga, zescape, be_ui_obj, ajax,
...
function g_switch_cb(){
    ...
    var on_off = be_bg_main.get('enabled') ? 'on' : 'off';
    set_user_cmd({label: 'g_switch', cmd: 'turn_'+on_off, country: ''});
    be_popup_lib.perr_ok({id: 'be_ui_click_'+on_off});
...
E.get_enabled_rule = function(url, opt){
    ...
    if (!opt.ignore_mitm_manual && is_mitm_active_manual())
        return {country: 'us', is_mitm: true, tab_id: tab_id};
    var stub_rules = E.be_rule.get('tabs_stub_rules')||{};
    if (be_util.is_stub_rule_enabled(stub_rules[tab_id], E.get_url(),
        E.be_ext.get('is_premium')))
    {
        return stub_rules[tab_id];
    }
...
E.init = function(ui_popup){
    ...
    E.listenTo(E.be_rule, 'set_tab_state', tab_state_cb);
...
var update_status = function(){
    ...
    var _new = [s1, s2, s3].includes('error') || !window.navigator.onLine ?
        'error' : [s1, s2, s3].includes('busy') ? 'busy' : 'ready';
...
var tab_state_cb = function(state){
    if (!state || state.tab_id != get_tab_id())
        return;
    if (!['fix_vpn_err', 'set_rule_err'].includes(state.curr))
        return;
    E.set_err('be_ui_vpn_tab_state', '', {state: state});
};
...
Protect_ui.prototype.init = function(){
    ...
    this.protect_pc = is_premium && E.be_vpn.get('protect_pc');
    this.protect_browser = is_premium && E.be_vpn.get('protect_browser');
    if (this.protect_pc)
...
Protect_ui.prototype.is_allowed = function(){
    return E.be_ext.get('is_premium') ||
        be_util.is_force_protect_mode(E.be_ext, E.get('country'), true);
};
...
function root2url(root_url){ return 'http://'+root_url+'/'; }

E.set_active_url = function(root_url, country, disable){
    var url = root2url(root_url);
    var rule_enabled;
    var tab_id = get_tab_id();
...
E.set_err = function(id, err, info){
    if (E.get('error'))
        return;
    E.set('error', id);
    zerr('be_ui_set_err %s %s', id, err ? err&&err.stack : '');
    be_popup_lib.err(id, '', err);
    be_popup_lib.perr_err({id: id, info: info, err: err});
    return etask([function(){
        return ui_lib.alert_error();
    }, function(){
        E.set('error', undefined);
        E.set('user.cmd', '');
    }]);
};
...
var menu_view = Backbone.View.extend({
    ...
    render: function(){
        ...
        var min_ver = (E.be_ext.get('bext_config')||{})
            .signin_redirect_back_to_site_min_ver;
        if (chrome && version_util.valid(min_ver) &&
            version_util.cmp(be_util.version(), min_ver)>=0)
        {
            $('#sign_in').on('click', function(e){
                e.preventDefault();
                be_util.set_login_redirect(E.get_host());
                chrome.tabs.update({url: 'https://hola.org/signin?'
                    +'utm_source=holaext&utm_content=menu&'
                    +'signin_redirect=true&next='+E.get_url()});
                window.close();
            });
        }
        if (is_lang)
...
function add_user_nav(){
    ...
    ui_lib.render_upgrade_link($react[0], {user_status: user_status,
        root_url: E.get_root()});
    if (E.be_ext.get('is_premium') || $('.popup-free').length)
        return;
    ...
    $('.popup-header-logo').on({mousemove: function(){
        ...
        var be_bg_main = window.popup_main&&window.popup_main.be_bg_main;
        if (be_bg_main)
            be_bg_main.fcall('gclid_ga_event', ['go_plus', 'header_logo']);
    }});
...
function add_trial_timer(){
    ...
    if (E.be_ext.get('is_premium') || !root_url)
        return;
    E.sp.spawn(etask({cancel: true}, [function(){
        ...
        E.listenTo(E.be_ext, 'trial_change', function(_root_url, t){
            if (E.get_root()==_root_url)
                render_timer();
        });
        render_timer();
    }]));
    ...
    return etask([function(){
        ...
        if (!disable)
        {
            ...
            return etask([function(){
                ...
                if (!hosts || host=='all_browser')
                    return;
                return set_hosts_rule({hosts: hosts, host: host,
                    enabled: true, proxy_country: proxy_country});
            }]);
        }
        if (is_mitm_active_manual())
        {
            hosts = (new Rules_set(E.get_root())).to_array().map(function(r){
                return zurl.add_proto(r); });
            set_user_cmd({label: 'set_mitm_disable', cmd: cmd,
                country: proxy_country});
            return E.be_vpn.fcall('mitm_manual_stop', [hosts, get_tab_id()]);
        }
        ...
        else
        {
            return etask([function(){
                ...
                if (be_vpn_util.is_all_browser(rule_enabled))
                    return;
                hosts = (new Rules_set(rule_enabled.name)).to_array()
                    .filter(function(n){ return n!=rule_enabled.name; });
                if (!hosts.length)
                    return;
                return set_hosts_rule({hosts: hosts, host: rule_enabled.name,
                    enabled: false});
            }]);
        }
...
function Rules_set(host, invert){
    var _this = this;
    this._data = {};
    this._invert = !!invert;
    if (!host)
        return;
    this.add(host);
    var id, sets_map = storage.get_json('be_rules_sets') || {};
    if (!(id = sets_map[host]))
        return;
    Object.entries(sets_map).forEach(function(e){
        if (e[1]==id)
            _this.add(e[0]);
    });
}
Rules_set.prototype.add = function(host){
    if (!host)
        return;
    this._data[host] = 1;
};
Rules_set.prototype.to_array = function(){
    var data = this._data;
    return Object.keys(data).filter(function(k){ return data[k]; });
};
Rules_set.prototype.save = function(){
    var _this = this;
    var sets_map = storage.get_json('be_rules_sets') || {};
    var id = +date();
    Object.keys(this._data).forEach(function(host){
        if (!!_this._data[host] ^ _this._invert)
            sets_map[host] = id;
        else
            delete sets_map[host];
    });
    storage.set_json('be_rules_sets', sets_map);
};

function set_hosts_rule(opt){
    var hosts = opt.hosts;
    var rules_set = new Rules_set(null, !opt.enabled);
    rules_set.add(opt.host);
    if (!opt.enabled)
    {
        return etask([function(){
            return etask.for_each(hosts||[], [function(){
                rules_set.add(this.iter.val);
                var rule = E.get_enabled_rule(root2url(this.iter.val));
                if (!rule)
                    return this.return();
                ...
                be_popup_lib.perr_err({id: 'be_ui_vpn_set_hosts_err',
                    ...
                    enabled: opt.enabled, country: proxy_country,
                    hosts: hosts}});
            }]);
        }, function(){ rules_set.save(); }]);
    }
    var proxy_country = opt.proxy_country.toLowerCase();
    return etask([function(){
        return etask.for_each(hosts||[], [function(){
            if (!check_before_unblock(this.iter.val))
                return this.return();
            ...
            be_popup_lib.perr_err({id: 'be_ui_vpn_set_hosts_err',
                info: {err: ''+err, root_url: this.iter.val,
                enabled: opt.enabled, country: proxy_country, hosts: hosts}});
        }]);
    }, function(){ rules_set.save(); }]);
}

function activate_mitm(opt){
    var host = opt.host, hosts = opt.hosts;
    E.protect_ui.set_enabled(false, undefined, undefined, true);
    set_user_cmd({label: 'set_mitm_enable', cmd: 'enable', country: 'mitm'});
    if (!host)
    {
        var rule_enabled = opt.rule_enabled;
        return etask([function(){
    ...
    return etask([function(){
        return etask.for_each((hosts||[]).concat([host]), [function(){
            ...
            var rule = E.get_enabled_rule(root2url(root),
                {ignore_mitm_manual: true});
            rules_set.add(root);
            if (!rule)
                return;
...
function init_tpopup(){
    if (!is_tpopup)
        return;
    var root_url = svc_util.get_root_url(zutil.get(window,
        'hola.tpopup_opt.url'));
    function close_hint_click(url){
        E.ui_popup.set_dont_show_again({root_url: url, period: 'default',
            src: 'x_tooltip', type: window.hola.tpopup_opt.type});
    }
    var $el = $('#tpopup_close').off('click').click(function(e){
        e.stopPropagation();
        var period = E.be_ext && E.be_ext.get('is_premium') ? 'default' :
            'session';
        E.ui_popup.set_dont_show_again({root_url: root_url,
    ...
    if (E.ui_popup && (E.ui_popup.is_watermark || E.ui_popup.is_mitm))
        return;
    ui_lib.render_close_hint({root_url: root_url, close_btn: $el[0],
        period: be_util.get_dont_show_def_period(E.be_ext),
        be_tpopup: E.ui_popup.be_tpopup, on_click: close_hint_click});
}

function uninit_tpopup(){
    if (!is_tpopup)
        return;
    $('#tpopup_close').off('click');
    ui_lib.remove_close_hint();
}
...
function needs_err_notify(status, cmd){
    if (status!='error')
        return false;
    if (cmd=='turn_on' && !E.get_enabled_rule())
        return false;
    return cmd && !['turn_off'].includes(cmd);
}
...
function state_cb(){
    ...
    if (!be_bg_main.get('agree_ts'))
        ...
        next = 'premium_popup';
    else if (E.skip_url() && !allow_skip_url())
        next = 'skip_url';
    else if (needs_err_notify(status, cmd))
    {
        next = curr;
        E.set_err('be_ui_vpn_render_state_err');
    }
...
function allow_skip_url(){
    return !!(E.be_ext.get('bext_config')||{}).stub_selection_for_skip_urls;
}

E.skip_url = function(opt){
    opt = opt||{};
    var url = opt.url || E.get_url();
    if (!url)
        return true;
    ...
    if (!opt.ignore_protect_pc && E.protect_ui.is_enabled_for_pc())
        return false;
    return !E.get_root() || be_vpn_util.is_skip_url(url);
};
...
function check_before_unblock(host){
    if (svc_util.get_root_domain(host)=='hola.org' && !allow_skip_url())
        return false;
    return true;
}
...
var country_selection_view_class = Backbone.View.extend({
    ...
    render: function(){
        ...
        {
            this.$el.toggleClass('is-mitm', true);
            this['p'+main] = 'mitm';
        ...
        {
            this.$el.toggleClass('is-mitm', false);
...
function on_get_plus_click(){
    be_util.open_tab({url: plus_ref('ext_protect',
        {root_url: E.get_root()}), force_new: true, force_active: true});
}
...
var unblock_protect_view_class = Backbone.View.extend({
    ...
    protect_on_click: function(e){
        E.loader.enable(true);
        be_popup_lib.perr_ok({id: 'be_ui_click_protect',
            info: {value: e.value}});
        if (!E.protect_ui.is_allowed())
...
var rating_view_class = Backbone.View.extend({
    ...
    on_click: function(e){
        ...
        if (!num)
            return;
        be_popup_lib.perr_ok({id: 'be_vpn_rating_rate', info: {rate: num}});
...
var rated_view_class = Backbone.View.extend({
    ...
    show: function(){
        ...
        if (this.rating==5 && !this.rate_on_store_ts)
            this.rate_on_store();
        else
            this.render();
    },
    rate_on_store: function(){
        ...
        be_util.open_new_tab({url: rate_url, force_active: true});
    },
    render: function(){
        if (this.opt.hidden)
            return;
    ...
    on_report_problem: ui_lib.open_report_problem,
    on_rate_us: function(){
...
var more_opt_view_class = Backbone.View.extend({
    ...
    on_no: function(e){
        e = e||{};
        set_user_cmd({label: 'more_opt', cmd: 'on_no', no_busy: true});
        var tab_id = get_tab_id();
        ...
        E.be_ext.fcall('trigger', ['not_working', {tab_id: tab_id,
            src: 'ui'}]);
    ...
    on_yes: function(){
        var _this = this;
        set_user_cmd({label: 'more_opt', cmd: 'on_yes', no_busy: true});
        var rule_enabled = E.get_enabled_rule();
    ...
    on_report: ui_lib.open_report_problem,
...
function unblock_on_click(e){
    E.loader.enable(true);
    be_popup_lib.perr_ok({id: 'be_ui_click_unblock', info: {value: e.value}});
...
function click_no_action(event, count){
    var tab_id = get_tab_id();
    E.sp.spawn(get_handler().fix_vpn());
    E.be_ext.fcall('trigger', ['not_working', {tab_id: tab_id, src: 'ui'}]);
    E.ui_popup.send_fix_it_report({
        rule: E.get_enabled_rule(),
        src_country: E.get('country'),
        event: event,
...
var react_enable_view_class = Backbone.View.extend({
    ...
    render: function(){
        ...
        ui_lib.render_enable_view(this.$el[0], {
            ...
            src_country: E.get('country'),
            report_action: ui_lib.open_report_problem,
            is_mitm: is_mitm_site() || is_mitm_active_manual(),
            type: 'protect',
            root_url: !skip && E.get_root(),
            is_protect_pc: E.protect_ui.is_enabled_for_pc(),
...
var enable_view_class = Backbone.View.extend({
    ...
    initialize: function(){
        ...
        var $container = this.$container = $('<div>',
            {class: 'popup-enabled-content'});
        var hover_title = this.hover_title = new title_view_class({flex: 1});
...
var redirect_view_class = Backbone.View.extend({
    ...
    initialize: function(opt){
        ...
        be_popup_lib.perr_ok({id: 'be_ui_redirect', info: {
            first: this.links[0], last: this.current_root,
            chain: this.links}});
    ...
    render: function(){
        ...
        var title_view = new title_view_class({
            ...
            sub_title_view: undefined&&$show_redirects, flex: 1});
        etask([function(){
...
E.script_set = function(rule, val){
    ...
    return etask([function(){
        return E.be_vpn.ecall('set_enabled', [true]);
    }, function(){
        ...
        new_rule.stub = E.skip_url() && allow_skip_url() ||
            !E.be_ext.get('is_premium') && be_util.is_google(val.root_url);
        if (!new_rule.enabled)
...
if (zutil.is_mocha())
    E.t = {set_country_rule: set_country_rule, B: B};
