...
define(['jquery', 'underscore', 'backbone', '/bext/pub/backbone.js',
    ...
    '/bext/vpn/pub/rule.js', '/util/zerr.js', '/util/storage.js',
    '/util/date.js', '/bext/vpn/pub/iframe.js', '/bext/vpn/pub/premium.js',
    '/util/util.js', '/bext/vpn/pub/util.js', '/bext/pub/locale.js',
    '/util/version_util.js', '/bext/vpn/pub/svc.js',
    '/bext/vpn/pub/trial.js', 'zon_config', 'conf'],
    ...
    zerr, storage, date, be_iframe, premium, zutil,
    be_vpn_util, T, version_util, be_svc, be_trial, zconf, conf){
...
function script_data(iframe_int, opt){
    ...
    function add_iframe(){
        ...
        var styles = {position: 'fixed', top: '5px', right: '20px',
            ...
            display: 'block'};
        if (['mitm_popup', 'watermark'].includes(opt.type))
        {
        ...
        var f = iframe_int.add({url: url, parent: get_iframe_parent()},
            styles);
        f.attr('id', '_hola_popup_iframe__');
...
E.do_tpopup = function(tab, tpopup_opt){
    ...
    if (popup_conf.disable || version_util.cmp(be_util.version(),
    ...
    return loading_tpopups[id] = loading_tpopups[id]||etask({async: true,
        ...
        if (need_force_suggestion(id, url))
        {
            ...
            be_tabs.set_force_suggestion(id, null);
            be_rule.set_stub_rule(id, null);
            return this.goto('render');
        }
        if (need_stub_rule_popup(id, url))
        {
            ...
            tpopup_type = 'suggestion';
            tpopup_country = be_rule.get_stub_rule(id).country.toUpperCase();
            be_tabs.set_force_suggestion(id, null);
            be_rule.set_stub_rule(id, null);
            return this.goto('render');
        }
        ...
        if (be_trial.get_trial_active(root_url) ||
            ...
            be_info.is_dont_show(id, root_url, 'suggestion'))
        {
        ...
        if (get_suggestion_mode(root_url)=='by_popular' &&
            !be_info.is_dont_show(id, root_url, 'suggestion'))
        {
            zerr.notice('tab:%d need watermark popup (by_popular)', id);
            tpopup_type = 'watermark';
            return this.goto('render');
        }
        var mode;
        ...
        {
            zerr.notice('tab:%d skip tpopup, geo_popup.mode is %s', id, mode);
            return this.return();
        }
    }, function render(e){
        ...
        var opt = {conf: conf, zon_config: zconf,
            ...
            root_url: root_url, url: url, ver: conf.version,
...
function get_geo_popup_mode(){
    ...
    return zutil.get(bext_conf, 'geo_popup.mode', 'by_popular');
}

function get_suggestion_mode(root_url){
    var site_conf = be_util.get_site_conf(be_ext, root_url);
    var conf = be_util.get_suggestion_conf(site_conf, be_info.get('country'));
    return conf && (conf.mode||'by_popular');
}
...
function need_geo_watermark(tab_id, url, root_url){
    var allow_hide = premium.is_active() ||
        zutil.get(be_ext.get('bext_config'), 'geo_popup.watermark.allow_hide');
    return be_util.is_geo_watermark(be_ext) && (get_rule(url)||{}).enabled &&
        (!allow_hide || !be_info.is_dont_show(tab_id, root_url, 'watermark'));
}
...
function need_stub_rule_popup(tab_id, url){
    if (!be_vpn_util.is_vpn_allowed(url, true) || be_vpn_util.is_skip_url(url))
        return false;
    var rule = be_rule.get_stub_rule(tab_id);
    var root_url = svc_util.get_root_url(url);
    return rule && !be_info.is_dont_show(tab_id, root_url, 'suggestion');
}
