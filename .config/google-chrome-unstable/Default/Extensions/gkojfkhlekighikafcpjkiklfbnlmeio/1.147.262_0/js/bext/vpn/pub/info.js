...
define(['jquery', 'underscore', 'backbone', '/bext/pub/backbone.js',
    ...
    '/bext/pub/lib.js', '/util/date.js', '/bext/vpn/pub/bg_ajax.js',
    '/util/storage.js', '/bext/vpn/pub/svc.js',
    '/bext/vpn/pub/mode.js', '/svc/hola/pub/svc_ipc.js', '/util/url.js',
    '/util/version_util.js', 'conf'],
    function($, _, Backbone, be_backbone, etask, be_ext, be_util, zutil, zerr,
    be_rule, B, be_lib, date, be_bg_ajax, storage, be_svc,
    be_mode, svc_ipc, zurl, version_util, conf){
B.assert_bg('be_info');
...
E.epopup_new = function(root_url){
    return etask([function(){ zerr.notice('epopup_new');
        ...
        return be_bg_ajax.ccgi_ajax({url: conf.url_ccgi+'/popup_new.json',
...
E.get_unblocking_rate = function(limit, country){
    ...
    return etask({name: 'get_unblocker_rate', cancel: true}, [function start(){
        ...
        return be_bg_ajax.ccgi_ajax({url: conf.url_ccgi+'/unblocking_rate',
            data: be_util.qs_ajax({src_country: country, limit: limit})});
    }, function(e){
        if (this.error)
        {
            if (n>1)
...
E.efetch_info = function(){
    var local_settings = !!storage.get_json('settings') ||
        version_util.cmp(storage.get('install_version'), '1.128.350')>0;
    return etask({name: 'efetch_info', cancel: true}, [function(){
        zerr.notice('be_info: fetch_info');
    }, function(){ 
        var src_country_fake = storage.get('src_country_fake')||'';
        return be_bg_ajax.ccgi_ajax({url: conf.url_ccgi+'/fetch_info',
        ...
        be_lib.perr_err({id: 'be_info_fetch_info_err', err: err});
        throw err;
    }]);
};
E.fetch_info = function(){
    if (zutil.is_mocha())
        return void E.efetch_info();
    E.trigger('fetch_info');
};
...
E.on('set_cid', function(){
    return E.sp.spawn(etask({name: 'set_cid'}, [function(){
        ...
        return be_bg_ajax.ccgi_ajax({timeout: 20000, method: 'POST',
            ...
            qs: {uuid: uuid, browser: be_util.browser(), cid: cid,
            ver: be_util.version()}});
...
E.on('set_user_id', function(){
    ...
    return etask('set_user_id', [function(){
        ...
        return etask.all({allow_fail: true}, {
            client: user_id && be_bg_ajax.ccgi_ajax({qs: be_ext.auth(),
...
E.set_dont_show_again = function(opt){
    ...
    if (opt.period=='default')
        opt.period = be_util.get_dont_show_def_period(be_ext);
...
function join_sessions(user){
    ...
    return etask('join_sessions', [function(){
        ...
        if (!(sync_token = be_svc.get('sync_token')))
            return this.return();
        return be_bg_ajax.hola_api_call('users/auth/token/join?token='+
            sync_token);
    }, function finally$(){
...
function get_user_sync_token(){
    return etask('get_user_sync_token', [function(){
        ...
        return be_bg_ajax.hola_api_call('users/auth/token/generate');
    }, function(data){
...
E.get_user_data = function(query){
    ...
    return etask('get_user_data', [function(){
        return be_bg_ajax.hola_api_call('users/get_user', query);
    }, function(res){
        last_res = res||{};
        if (last_res.user)
        {
        ...
        if (!(sync_token = be_svc.get('sync_token')))
            return this.return(last_res);
        return be_bg_ajax.hola_api_call('users/auth/token/login?token='+
...
E.resend_verification_email = function(){
    return be_bg_ajax.hola_api_call('users/send_email_verification',
        {method: 'POST'});
};
