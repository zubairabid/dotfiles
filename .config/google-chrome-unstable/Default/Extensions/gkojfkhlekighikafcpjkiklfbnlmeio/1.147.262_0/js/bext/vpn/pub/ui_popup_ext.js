...
function is_suggestion_popup(){ return get_tpopup_type()=='suggestion'; }
function is_vpn(){
...
E.render_popup = function(opt){
    ...
    else if (is_suggestion_popup())
        E.render_suggestion(opt);
    else
        render_vpn();
};
...
E.render_suggestion = function(opt){
    E.is_suggestion = true;
    init_first_render();
    watermark.init(E);
    var country = zutil.get(window, 'hola.tpopup_opt.country');
    if (country)
        watermark.render({suggest_country: country, prefix: 'stub_rule'});
    else
        watermark.render({force_suggestion: true, prefix: 'suggest'});
    E.be_popup_main.show_tpopup();
};
...
function update_logo(){
    ...
    etask([function(){
        return root_url && E.be_trial.ecall('get_trial_active', [root_url]);
    }, function(trial){
...
E.init = function(){
    try {
        ...
        E.be_tabs = B.backbone.client.start('be_tabs');
        ...
        E.be_bg_ajax = B.backbone.client.start('be_bg_ajax');
        be_util.set_ajax_cb(function(req){
            return E.be_bg_ajax.ecall('perr_ajax', [req]); });
...
E.uninit = function(){
    try {
        ...
        if (E.R)
        {
            ...
            B.backbone.client.stop('be_tabs');
            B.backbone.client.stop('be_info');
            B.backbone.client.stop('be_bg_ajax');
            be_util.set_ajax_cb();
            if (is_vpn())
            {
...
E.send_fix_it_report = function(opt){
    ...
    if (opt.send_logs && be_bg_main)
    {
        ...
        if ((debug_domains = zutil.get(config, 'debug_logs.domains')) &&
            debug_domains.includes(root_url) || E.be_ext.get('gen.dbg_log_on')
            || Math.random()*100<zutil.get(config,
            'debug_logs.all_domains_perc', 0))
        {
