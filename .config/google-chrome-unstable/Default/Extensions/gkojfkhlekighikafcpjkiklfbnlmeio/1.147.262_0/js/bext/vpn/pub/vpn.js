...
define(['jquery', 'underscore', 'backbone', '/bext/pub/backbone.js',
    ...
    '/bext/vpn/pub/bext_config.js', 'conf'],
    function($, _, Backbone, be_backbone, etask, string, be_ext,
    zutil, array, be_util, zerr,
    ...
    be_agent, be_svc, be_pac, be_mode, be_rules, be_tab_perr,
...
E.uninit = function(){
    ...
    if (chrome && B.have['web_request.on_auth_required'])
        chrome.webRequest.onAuthRequired.removeListener(zagent_conn_log_cb);
    ...
    if (zutil.is_mocha)
    {
        E.be_tpopup.uninit();
        E.be_rule.uninit();
        E.be_tab_unblocker.uninit();
        E.be_tab_perr.uninit();
        E.be_agent.uninit();
        E.be_pac.uninit();
        E.be_svc.uninit();
        E.be_mode.uninit();
...
proxy_settings_monitor.init = function(){
    if (this.inited || !chrome || conf.firefox_web_ext || zutil.is_mocha())
        return;
    this.inited = true;
...
proxy_settings_monitor.changed_cb = function(details){
    ...
    be_ext.set({'proxy.effective.control_level': details.levelOfControl,
...
E.init = function(){
    ...
    E.listenTo(be_ext, 'trial_end', trial_end_cb);
    ...
    if (chrome && B.have['web_request.on_auth_required'])
    {
        chrome.webRequest.onAuthRequired.addListener(zagent_conn_log_cb,
            {urls: ['<all_urls>']});
    }
    ...
    var bump_ts;
    if (!(bump_ts = storage.get_int('bump_ts')))
        return;
    storage.clr('bump_ts');
    be_lib.perr_ok({id: 'be_bump_done',
        info: {bump: storage.get_int('bump'), ms: Date.now()-bump_ts}});
};
...
function check_bump(bext_config){
    if (E.bump_active || !bext_config.bump)
        return;
    var bump;
    if (!(bump = storage.get_int('bump')) || config_stats.n==1)
        return void storage.set('bump', bext_config.bump);
    if (bext_config.bump<=bump)
        return;
    E.bump_active = true;
    storage.set('bump', bext_config.bump);
    E.listen_to(be_tab_unblocker, 'change:activity', _.debounce(function(){
        ['agent_key', 'agent_key_ts', 'popup_rating_cache']
        .concat(bext_config.bump_clear||[])
        .forEach(function(k){ storage.clr(k); });
        storage.set('bump_ts', Date.now());
        be_lib.reload_ext();
    }, bext_config.bump_idle||5*date.ms.MIN));
}
...
function set_bext_config(bext_config){
    ...
    if (protect_ui && (!protect_ui.min_version ||
        version_util.cmp(be_util.version(), protect_ui.min_version)>=0))
    {
        ...
        E.set('protect_pc', be_util.os_win() &&
            ...
            version_util.cmp(be_util.version(), protect_pc_min_ver)>=0));
...
function bext_config_init(){
    ...
    [function try_catch$loop(){
        ...
        return be_bg_ajax.ccgi_ajax({timeout: 20000, method: 'GET', url: url});
    }, function(e){
        config_stats.n++;
        ...
        if (err || !e)
        {
            ...
            zutil.get(be_ext.get('bext_config'), 'stamp', 0) > e.stamp)
        {
            be_lib.perr_ok({id: 'be_bext_config_old',
                info: {n: config_stats.n, after_err: config_stats.err,
                attempt: config_stats.attempt}});
        }
        else
...
E.update_vpn_countries = function(){
    return etask([function try_catch$(){
        return be_bg_ajax.ccgi_ajax({timeout: 20000, method: 'GET',
            ...
            be_util.browser()+'&ver='+be_util.version()});
    }, function(res){
...
function first_run(active){
    ...
    if (!details)
        return;
    be_ext.set('install_details', null);
    ...
    var skip_first_run = !!(svc_install || !install || ext_skip_first_run);
    if (skip_first_run)
    {
...
function get_post_instll_url(){
    return zescape.uri('https://hola.org/unblock/install',
        ...
        uuid: get_bg_main().get('uuid')}, be_ext.auth()));
}

function zagent_conn_log_cb(details){
...
function tz_spoof_inject(o){
    ...
    var site, url = zutil.get(o, 'info.url');
    if (!(site = E.get_site_conf(url)) || !site.tz_spoof)
        return;
    var fn = function(offset){
        var script = document.createElement('script');
        script.src = chrome.runtime.getURL('js/bext/vpn/pub/tz_spoof.js?'+
            'offset='+offset);
        document.head.appendChild(script);
    };
    var code = '('+fn.toString()+')('+site.tz_spoof+')';
    var tabapi = B.tabid2api(o.id);
    etask.cb_apply(B.tabs, '.execute_script', [tabapi, {code: code}]);
...
E.set_enabled = function(on){
    try {
        ...
        if (!!be_ext.get('r.ext.enabled')!=on &&
            !be_ext.get('ext.conflict'))
        {
            var attributes = zutil.clone(be_ext.attributes);
...
E.mitm_manual_stop = function(urls, tab_id, no_reload){
    array.to_array(urls).forEach(function(url){
...
function trial_end_cb(root_url){
    var tabs = be_tabs.get_nav_tabs();
    Object.keys(tabs).forEach(function(tab_id){
        var url = tabs[tab_id];
        if (svc_util.get_root_url(url)==root_url &&
