...
define(['jquery', 'underscore', 'backbone', '/bext/pub/backbone.js',
    ...
    '/bext/vpn/pub/trial.js', '/util/version_util.js',
    '/util/rate_limit.js'],
    ...
    hybrid_mock, premium, storage, ajax, be_info, mitm_lib,
    be_svc, be_premium, be_trial, version_util, rate_limit){
B.assert_bg('be_tab_unblocker');
function get_bg_main(){ return window.be_bg_main; }
var assign = Object.assign, chrome = window.chrome;
var proxy_debug, proxy_debug_timing, debug_hooks;
...
function get_site_verify_conf(req, rule){
    ...
    if (req.method!='GET')
        return false;
    return be_util.get_site_verify_conf(be_ext, rule.name, req.url,
        rule.country);
}
...
class Base_handler {
    constructor(){
        ...
        this.on_completed_debug = ff_cb_wrapper('on_completed_debug', d=>{
            ...
            if (!req)
                return;
            var rule = be_vpn_util.get_rule_min_fmt(req.opt.rule);
        ...
        this.on_headers_received_media = ff_cb_wrapper(
            ...
            var route = be_vpn_util.gen_route_str_lc(req.route_opt,
                {no_algo: true});
            E.be_rule.fix_media_req_err(zutil.get(req, 'opt.rule'),
                req.agent, d, tab, route, media_failure);
        });
        this.on_headers_received_browse = ff_cb_wrapper(
            ...
            var tab_url = d.initiator||d.url, root_url;
            if (tab_url)
    ...
    _on_first_request(details, opt){
        ...
        if (!req)
        {
            ...
            req.strategy = unblocker_lib.handle_request(url, {
                ...
                pool: pool,
                strict: strict,
                force_media_direct: be_ext.get('gen.is_media_direct_on'),
            });
            ...
            var t = get_tab_unblocker(details.tabId);
            if (t)
            {
                if (proxy_debug)
                {
    ...
    _on_headers_received(details){
        ...
        if (hola_warn && tab_unblocker && !tab_unblocker.rule.changing_proxy)
        {
            var route = be_vpn_util.gen_route_str_lc(req.route_opt,
                {no_algo: true});
            E.be_rule.change_proxy(details, hola_warn, route);
        }
    ...
    _unsafe_tab_unblocker_end_cb(details){
        ...
        var tabid = details.tabId, tab_unblocker = get_tab_unblocker(tabid);
        if (tab_unblocker)
        {
        ...
        {
            var req_info = mitm_lib.tab_get_req_info(tabid)||{};
        ...
        var errors = zutil.get(bext_config, 'request_errors.handle', [])
            .slice();
        if (!be_ext.get('gen.disable_tunnel_error') && is_main_frame(details))
...
class Ff_new_handler extends Base_handler {
    ...
    gen_proxy_req(req, details, cmd, country){
        var prot = zutil.get(req, 'opt.prot', 'https');
        req.route_opt = {prot: prot, country: country, pool: cmd.proxy.pool};
        if (cmd.proxy.peer)
...
class Chrome_handler extends Base_handler {
    ...
    gen_proxy_req(req, details, cmd, country){
        ...
        var prot = zutil.get(req, 'opt.prot', 'https').toUpperCase();
        req.route_opt = {prot: prot, country: country, pool: cmd.proxy.pool};
        if (cmd.proxy.peer)
            req.route_opt.peer = true;
        var proxy = get_proxy_agent(req);
        ...
        var proxy_str = req.route_opt.prot+' '+(host||proxy.host||proxy.ip)+
            ':'+proxy.port;
...
function get_proxy_agent(req, dry_run){
    ...
    if (!dry_run)
    {
        ...
        {
            var info = mitm_lib.tab_get_req_info(req.tab_id)||{};
            if (!info.agents)
                info.agents = {};
    ...
    var port = be_trial.get_trial_agent_port(rule, agent.port, req.route_opt);
...
E.get_tab_unblocker = get_tab_unblocker;
    ...
    fix_url)
{
    ...
    tab_unblocker = {country: rule_info.rule.country, rule: rule_info.rule,
        ...
        be_util.get_site_conf(be_ext, root_url||rule_info.rule.name)||{}};
...
E.update_rule_urls = function(rules){
    ...
    return etask('update_rule_urls', [function(){
        ...
        for (var r in rules.unblocker_rules)
        {
            ...
            var pool = be_agent.is_pool_rule(rule) ? '.pool' : '';
            def.forEach(function(b){
                b = assign({country: rule.country, link: rule.link,
                    name: rule.name}, b);
                var s = be_vpn_util.gen_route_str_lc(b)+pool;
                if (!agents[s])
                {
                    ag_rules.push(b);
                    agents[s] = true;
...
E.ajax_via_proxy = function(url, _opt, state){
    ...
    return etask({name: 'ajax_via_proxy', cancel: true}, [function(){
        var rule = opt.rule;
        _this = this;
        if ((rule || _opt.force=='proxy') && !_opt.country && !_opt.agent)
        {
...
function get_log(){
    return get_bg_main().get_log(be_ext.get('is_premium')); }
var on_tab_completed = cb_wrapper(function(o){
        ...
        (trace = be_tabs.get_trace(o.tabId)))
    {
        ...
        if (!r.length)
            return;
        info = {url: o.url, autoreload_n: r.length, errors: []};
        r.forEach(function(e){ info.errors.push(e.error); });
        if (be_ext.get('gen.is_tab_trace_on'))
            info.trace = trace;
        if (be_ext.get('gen.is_log_on'))
            info.log = get_log();
        be_lib.perr_err({id: 'tab_autoreload_completed', info: info,
            rate_limit: {count: 5}});
    }
    var start_ts, load_perr;
    if (!(start_ts = tab_loads[o.tabId]) ||
        !(load_perr = zutil.get(bext_config, 'load_perr')))
    {
        return;
    }
    var _url = zurl.parse(o.url), _path = _url.path.toLowerCase();
    var root_url = svc_util.get_root_url(o.url), rule, path;
    if (!(rule = load_perr.find(function(r){ return r.domain==root_url; })) ||
        (path = rule.paths.find(function(p){
        return _path.startsWith('/'+p); }))===undefined)
    {
        return;
    }
    info = {id: rule.id, path: path||'root', ms: Date.now()-start_ts,
        loc: zutil.get(be_info.get('location'), 'country', '').toLowerCase()};
    var tab;
    if ((tab = get_tab_unblocker(o.tabId)) && (rule = tab.rule))
        assign(info, {country: rule.country, trial: rule.src=='trial'});
    else
        info.local = true;
    be_lib.perr_ok({id: 'tab_load_completed', info: info});
});
...
function get_enabled_rule(url){
    var key = 'http://'+svc_util.get_root_url(url)+'/';
    if (zutil.is_mocha() && test_rules[key])
        return test_rules[key];
    var rules = E.be_rule.get_rules(key);
    var r = rules && rules[0];
...
var init_mitm_check_url = ff_cb_wrapper('init_mitm_check_url', details=>{
    var url = details.url;
    if (!url || url.startsWith('https') || !be_vpn_util.is_vpn_allowed(url,
...
var init_mitm_check_err = cb_wrapper(function(o){
    ...
    if (force_strategy!='peer' && geo_rule
        || !is_unblock_allowed(tab_id, tab_url)
        || !be_vpn_util.is_vpn_allowed(tab_url, true, browser.isInNet)
    ...
    return etask('init_mitm_check_err', [function(){
        ...
        return mitm_lib.check_mitm_blocking({url: tab_url, tab_url: tab_url,
            ...
            force_peer: geo_rule && geo_rule.country, err_str: o.error,
            agents: agents});
    }, function(ret){ handle_mitm_block(tab_url, tab_id, ret); }]);
});
...
var init_mitm_before_request = ff_cb_wrapper('on_before_request_int',
    on_before_request_int);
var init_mitm_on_req_end = ff_cb_wrapper('on_completed_int',
    on_completed_request_int);
var init_mitm_on_send_headers = ff_cb_wrapper('init_mitm_on_send_headers',
    on_send_headers_int);
var init_mitm_on_headers = ff_cb_wrapper('init_mitm_on_headers',
    on_headers_int);
...
function on_not_working(e){
    ...
    tab.not_working = {ts: Date.now(), info: e, stats: {},
        rule: be_vpn_util.get_rule_min_fmt(tab.rule)};
    monitor_not_working(e.tab_id);
    if (e.src!='ui' || !debug_hooks)
        return;
    let errors = {}, total;
...
E.init = function(opt){
    ...
    if (window && window.hola)
        window.hola.get_log = function(){ return get_log().join('\n'); };
};

var init_bg_ajax_on_before_request = ff_cb_wrapper('on_before_bg_ajax',
    on_before_request_int);
var init_bg_ajax_on_req_end = ff_cb_wrapper('on_req_end_bg_ajax',
    on_completed_request_int);
var init_bg_ajax_on_send_headers = ff_cb_wrapper('on_send_headers_bg_ajax',
    on_send_headers_int);
var init_bg_ajax_on_headers = ff_cb_wrapper('on_headers_bg_ajax',
    on_headers_int);
E.init_bg_ajax_via_proxy = function(){
    if (E.ajax_via_proxy_inited || zutil.is_mocha())
        return;
    E.ajax_via_proxy_inited = 1;
    E.url_to_rule_infos = E.url_to_rule_infos||{};
    if (!requests_handler)
        requests_handler = Base_handler.get_handler();
    be_pac.init();
    var et = be_pac.load_pac_file(undefined, true);
    var urls = ['http://internal.hola/*', 'https://client.hola.org/*',
        'https://hola.org/*', 'https://perr.hola.org/*',
        'https://client.svd-cdn.com/*', 'https://svd-cdn.com/*',
        'https://perr.svd-cdn.com/*'];
    chrome.webRequest.onBeforeRequest.addListener(
...
E.uninit_bg_ajax_via_proxy = function(){
    if (!E.ajax_via_proxy_inited)
        return;
    E.ajax_via_proxy_inited = 0;
    chrome.webRequest.onBeforeRequest.removeListener(
        init_bg_ajax_on_before_request);
    chrome.webRequest.onErrorOccurred.removeListener(init_bg_ajax_on_req_end);
...
function update_config(){
    ...
    proxy_debug_timing = zutil.get(bext_config, 'debug.proxy_debug_timing');
...
E.get_all_agents = function(){
    ...
    if (!E.rules)
        return agents;
    agents = agents.concat(be_agent.get_all_agents('us')||[]);
    for (var r in E.rules.unblocker_rules)
    {
        var rule = E.rules.unblocker_rules[r];
        unblocker_lib.get_rule_routes(rule).forEach(function(s){
            if (s!='us')
                agents = agents.concat(be_agent.get_all_agents(s)||[]);
        });
    }
    return agents;
};
...
if (zutil.is_mocha())
{
    E.t = {Base_handler, is_force_proxy, on_not_working, tab_unblocker_del,
        ...
        on_tab_created, on_tab_removed, get_tab_unblocker};
}
