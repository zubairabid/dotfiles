...
class Base_handler {
    constructor(){
        ...
        this.on_headers_received_media = ff_cb_wrapper(
            ...
            if (!set_err)
                return;
            if (tab.set_err.agent==agent)
                tab.set_err.count++;
            else
            {
                tab.set_err.count = 1;
                tab.set_err.agent = agent;
            }
            if (!req.agent.pool || tab.set_err.count>3)
            {
                zerr.debug('tab:%d try agent replacement instead of set err: '
                    +(req.agent.pool ? 'limit exceeded' : 'non-pool agent'));
                if (this.check_retry(req, d, tab) &&
                    media_failure.cancel_to_retry)
                {
                    return d.ret = {cancel: true};
                }
                return;
            }
            if (media_failure.disable_set_err)
                return;
            var limit = assign({}, media_failure.set_err_rate_limit,
                {ms: 5*date.ms.MIN, count: 10});
    ...
    tab_unblocker_cb(details){
        ...
        if (is_main)
        {
            ...
            if (tab_unblocker && rule_info && rule_info.rule.src=='trial' &&
                (site = be_util.get_site_conf(be_ext, rule_info.rule.name)) &&
    ...
    _unsafe_tab_unblocker_end_cb(details){
        ...
        if (this.check_retry(req, details, tab_unblocker))
            return;
        var curr;
            ...
            (curr = get_proxy_agent(req, true)) && req.agent.host!=curr.host)
        {
            zerr.notice('tab:%d ignore agent change: failed %s current %s',
                tabid, req.agent.host, curr.host);
            return;
        }
    ...
    check_retry(req, details, tab_unblocker){
        ...
        var retry = function(reason){
            ...
            zerr.notice('tab:%d allow retry for %s failed request, %s %s',
                ...
                req.url.slice(0, 200));
            be_lib.perr_err({id: 'req_err_retry_'+reason, info: {url: req.url,
                type: details.type, method: req.method, error: details.error,
                status: details.statusCode, agent: req.agent,
                rule: get_rule_min_fmt(req.opt.rule)}});
...
class Ff_new_handler extends Base_handler {
    ...
    gen_proxy_req(req, details, cmd, country){
        ...
        details.hola_proxy = {type: req.route_opt.prot,
            host: proxy.host||proxy.ip, port: proxy.port};
    }
...
class Chrome_handler extends Base_handler {
    ...
    gen_proxy_req(req, details, cmd, country){
        ...
        req.proxy_req = true;
        var proxy_str = req.route_opt.prot+' '+(proxy.host||proxy.ip)+':'+
            proxy.port;
...
function get_log(){
    return be_bg_main && be_bg_main.get_log(be_ext.get('is_premium')); }
var on_tab_completed = cb_wrapper(function(o){
    ...
    var tab, load_perr;
    if (!(tab = get_tab_unblocker(o.tabId)) || !tab.rule ||
    ...
    be_lib.perr_ok({id: 'tab_load_completed', info: {id: rule.id,
        path: path||'root', ms: Date.now()-tab.ts,
        country: tab.rule.country}});
});
...
var on_before_navigate = cb_wrapper(function(o){
    ...
    load_errors[o.tabId] = {};
    var tab;
    if (tab = get_tab_unblocker(o.tabId))
        tab.ts = Date.now();
    if (mitm_lib.inited)
...
if (zutil.is_mocha())
{
    E.t = {Base_handler, is_force_proxy, on_not_working, tab_unblocker_del,
        ...
        get_forced_strategy, test_rules: test_rules};
}
