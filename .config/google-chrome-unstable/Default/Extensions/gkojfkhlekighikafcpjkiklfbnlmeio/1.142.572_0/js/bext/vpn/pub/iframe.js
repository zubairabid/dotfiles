...
function iframe_int(){
    ...
    function get_pos_css($el){
        ...
        return {position: 'fixed', zIndex: 2147483647, height: rect.height,
    ...
    function animate_iframe($el, css, time, no_scale_anim){
        ...
        if (JSON.stringify(prev_pos)==JSON.stringify(next_pos))
            return;
        $el.css({opacity: 0});
        var $dummy = $('<div>').css(prev_pos).css({background: 'white'})
            .insertAfter($el);
    ...
    E_int.resize = function(opt){
        ...
        {
            var style = $frame[0].style;
            Object.entries(css).forEach(function(prop){
                var val = prop[1]+'';
                if (val.match(/^\d*$/))
                    val += 'px';
                style.setProperty(prop[0], val, 'important');
            });
        }
