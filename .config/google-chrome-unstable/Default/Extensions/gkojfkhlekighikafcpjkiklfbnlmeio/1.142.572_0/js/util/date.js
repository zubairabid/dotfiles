...
E.strftime = function(fmt, d, opt){
    ...
    function replace(fmt){ return fmt.replace(/%([-_0]?.)/g, function(_, c){
        ...
        switch (c)
        {
        case 'A': return locale.days_long[l.getDay(d)]; 
        case 'a': return locale.days_short[l.getDay(d)]; 
        case 'B': return locale.months_long[l.getMonth(d)]; 
        case 'b': return locale.months_short[l.getMonth(d)]; 
        case 'C': 
            return padx(Math.floor(l.getFullYear(d)/100), padding);
        case 'D': return replace(formats.D || '%m/%d/%y'); 
        case 'd': return padx(l.getDate(d), padding); 
        case 'e': return l.getDate(d); 
        case 'F': return replace(formats.F || '%Y-%m-%d'); 
        case 'H': return padx(l.getHours(d), padding); 
        case 'h': return locale.months_short[l.getMonth(d)]; 
        case 'I': return padx(hours12(l.getHours(d)), padding); 
        case 'j': 
            day = Math.ceil((+d-l.getYearBegin(d))/(1000*60*60*24));
            return pad(day, 3);
        case 'k': 
            return padx(l.getHours(d), padding===undefined ? ' ' : padding);
        case 'L': return pad(Math.floor(d.getMilliseconds()), 3); 
        case 'l': 
            return padx(hours12(l.getHours(d)),
                padding===undefined ? ' ' : padding);
        case 'M': return padx(l.getMinutes(d), padding); 
        case 'm': return padx(l.getMonth(d)+1, padding); 
        case 'n': return '\n'; 
        case 'o': return ''+l.getDate(d)+ord_str(l.getDate(d)); 
        case 'P': 
            return (l.getHours(d)<12 ? locale.AM : locale.PM).toLowerCase();
        case 'p': return l.getHours(d)<12 ? locale.AM : locale.PM; 
        case 'R': return replace(formats.R || '%H:%M'); 
        case 'r': return replace(formats.r || '%I:%M:%S %p'); 
        case 'S': return padx(l.getSeconds(d), padding); 
        case 's': return Math.floor(+d/1000); 
        case 'T': return replace(formats.T || '%H:%M:%S'); 
        case 't': return '\t'; 
        case 'U': return padx(week_num(l, d, 'sunday'), padding); 
        case 'u': 
            day = l.getDay(d);
            return day==0 ? 7 : day;
        case 'v': return replace(formats.v || '%e-%b-%Y'); 
        case 'W': return padx(week_num(l, d, 'monday'), padding); 
        case 'w': return l.getDay(d); 
        case 'Y': return l.getFullYear(d); 
        case 'y': return (''+l.getFullYear(d)).slice(-2); 
        case 'Z': 
            if (utc)
                return 'GMT';
