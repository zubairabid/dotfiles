...
E.handle_request = function(url, opt){
    ...
    {
        ex = pac_engine.find_proxy_for_url_exception(url.orig, url.hostname);
        strategy = get_strategy(strategy, ex);
    }
    if (strategy.cache)
    {
...
function ConnectivityCheck(opt){
    ...
    for (var name in this.states)
    {
        ...
        if (!state.urls || state.urls.length)
            continue;
        state.urls.push(!opt[name] || name.endsWith('_agents') ? opt.hola :
            opt[name]);
    }
    this.opt = opt;
    this.logs = [];
...
ConnectivityCheck.prototype.check = function(state, loop_et){
    ...
    function send_ajax(url){
        ...
        ts = Date.now();
        var et = _this.api.ajax_via_proxy(_url, opt);
        if (agent)
    ...
    var et = etask('connectivity_check', [function(){
        ...
        return this.wait_child('any', function(resp){
            ...
            if (!_ret.error && !proxy_err)
            {
                if (next)
                {
                    _this.api.perr({id: 'connectivity_check_failed_agent',
        ...
        if (!ret.error)
        {
            ...
            {
                _this.selected = state;
                if (agents.length)
                    _this.selected = assign({agent: agents[next]}, state);
                _this.api.perr({id: 'connectivity_check_success',
        ...
        {
            _this.api.perr({id: 'connectivity_check_failed',
                rate_limit: {count: 5}, info: {name: state.name,
                urls: state.urls}});
        }
        loop_et.continue();
...
ConnectivityCheck.prototype.run = function(opt){
    ...
    [function(){
            ...
            agents instanceof ConnectivityCheck)
        {
            if (agents.selected)
            {
                _this.selected = assign({agent: agents.selected.agent}, state);
                return;
            }
            this.set_state('loop');
            return agents.run();
        ...
        {
            _this.api.perr({id: 'connectivity_check_all_failed',
...
ConnectivityCheck.prototype.get_backend_link = function(){
    var s;
    return (s = this.selected) ? {url: s.urls[0], agent: s.agent} :
