...
E.init_globals = function(root, country_code, link, text){
    link = link||'https://hola.org/unblock/popular?utm_source=holaext';
    var country = country_code ? T(country_code.toUpperCase()) : '';
    var root_site = root||T('Unblock cool sites');
    var site = country ? T('$1 from $2', [root_site, country]) : root_site;
    var subject = root ? T('Unblock $1 - cool site!', [site]) : site;
    ...
    if (torch)
    {
        ...
        assign(E.share.twitter, {text: T('Check out Hola for $1!',
            ['@TorchBrowser'])+T('I can now unblock $1 from any country!',
            [root])+' '+torch_url});
        ...
        assign(E.share.email, {
            ...
            body: T('Hi,\n\nI started using $1 ($2). It lets me surf '
                +'the Internet faster and unblock $3 in my country.',
...
E.QuickShare = Backbone.View.extend({
    ...
    msg_twitter: {text: T('I just unblocked a censored site using Hola for'+
        '$1!', ['@TorchBrowser'])+T('Try it')+': '+torch_url},
    msg_facebook: {
        s: 100,
        'p[url]': torch_url,
        'p[images][0]': 'https://hola.org/img/torch-logo.jpg',
        'p[title]': T('I just unblocked a censored site using Hola for $1!',
            ['Torch Browser!']),
        'p[summary]': T('Unblock censored sites, try it now: $1', [
            torch_url])
    }
});
