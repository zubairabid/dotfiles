...
define(['jquery', 'underscore', 'backbone', '/bext/pub/backbone.js',
    ...
    be_iframe, vstat, be_premium, force_lib, pcountries, be_trial){
B.assert_bg('be_vpn');
...
function monitor_active_init(){
        ...
        cancel: true}, [function loop()
    {
        ...
        var active_ts_ar = [data.unblocker_tab_ts, data.ui_close_ts,
            data.ui_open_ts, data.req_ts];
        if (ts_in_range(begin_ts, active_ts_ar, end_ts))
        {
            var agents = [];
            for (var host in data.agents)
            {
                if (ts_in_range(begin_ts, [data.agents[host]], end_ts))
                    agents.push(host);
            }
            be_lib.perr_ok({id: 'be_active', info: {agents: agents,
                begin: begin_ts, end: end_ts, req_ts: data.req_ts,
                unblocker_tab_ts: data.unblocker_tab_ts,
        ...
        var tabs = Object.values(be_tab_unblocker.tab_unblockers||{});
        if (!tabs.length)
            return this.goto('loop');
        var data = storage.get_json('monitor_active')||{};
        data.unblocker_tab_ts = Date.now();
        var expire_ts = data.unblocker_tab_ts-period;
        data.agents = data.agents||{};
        var req_ts = 0;
        tabs.forEach(function(tab){
            if (!tab || !tab.last_req_ts)
                return;
            req_ts = Math.max(req_ts, tab.last_req_ts);
            var to_remove = [];
            for (var host in tab.agents)
            {
                if (tab.agents[host]>expire_ts)
                    data.agents[host] = tab.agents[host];
                else
                    to_remove.push(host);
            }
            for (var i=0; i<to_remove.length; i++)
                delete data.agents[to_remove[i]];
        });
        data.req_ts = req_ts||undefined;
        storage.set_json('monitor_active', data);
        return this.goto('loop');
    }, function finally$(){
...
E.script_set = function(rule, val){
    ...
    return etask({name: 'script_set', cancel: true}, [function(){
        ...
        var site_conf = be_util.get_site_conf(be_ext, val.root_url);
        if (val.enabled && site_conf && site_conf.require_plus &&
            !be_ext.get('is_premium') && site_conf.trial &&
            !be_trial.get_trial_active(val.root_url))
        {
            be_info.set_site_storage(val.root_url, 'force_trial',
                {country: val.country||rule.country});
            return this.return();
        }
