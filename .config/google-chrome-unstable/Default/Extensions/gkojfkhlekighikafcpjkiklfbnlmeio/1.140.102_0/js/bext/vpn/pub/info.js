...
E.on('set_user_id', function(){
    ...
    return etask('set_user_id', [function(){
        ...
        return etask.all({allow_fail: true}, {
            client: user_id && ajax.json({qs: be_ext.auth(), method: 'POST',
...
function join_sessions(user){
    ...
    return etask('join_sessions', [function(){
        ...
        if (!(sync_token = be_svc.get('sync_token')))
            return this.return();
        return ajax.hola_api_call('users/auth/token/join?token='+sync_token);
    }, function finally$(){
...
function get_user_sync_token(){
    return etask('get_user_sync_token', [function(){
        ...
        return ajax.hola_api_call('users/auth/token/generate');
    }, function(data){
...
E.get_user_data = function(query){
    ...
    return etask('get_user_data', [function(){
        return ajax.hola_api_call('users/get_user', query);
    }, function(res){
        last_res = res||{};
        if (last_res.user)
        {
        ...
        if (!(sync_token = be_svc.get('sync_token')))
            return this.return(last_res);
        return ajax.hola_api_call('users/auth/token/login?token='+sync_token,
            {method: 'POST'});
    }, function(){
        if (this.error)
            return this.return(last_res);
