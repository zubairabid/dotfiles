...
define(['/bext/pub/backbone.js', '/bext/pub/browser.js', '/util/etask.js',
    ...
    be_util, be_premium, zutil, svc_util, zurl, be_vpn_util, be_agent){
...
function TrialLocal(){
    this.trials = [];
    this.name = 'local';
    this.get_trials = get_trials.bind(this, '/trial_get');
}

TrialLocal.prototype.monitor_active_trial = function(){
    if (this.trial_monitor)
        return;
    var _this = this;
    this.trial_monitor = etask({async: true}, [function(){
        return monitor_active_trial(function(is_active){
...
TrialLocal.prototype.set_trial = function(root_url, cmd, opt){
    opt = opt||{};
    cmd = cmd||'start';
    var _this = this;
    return etask('start_trial', [function(){
        ...
        be_lib.perr_ok({id: 'be_set_trial_got_trial', info: {cmd: cmd,
            root_url: root_url, time_left: res.time_left||0,
            grace_ms: res.grace_ms||0}});
        if (!opt.dry_run)
            be_ext.trigger('trial_start', root_url);
        _this.trials = update_trial(res.trials);
...
TrialLocal.prototype.is_trial_available = function(root_url){
    var site;
    if (!root_url || !be_ext.get('user_id') ||
        !(site = be_util.get_site_conf(be_ext, root_url)) || !site.trial ||
        this.get_trial_active() || this.is_trial_waiting(root_url))
    {
        return;
    }
    var t;
...
TrialLocal.prototype.is_trial_waiting = function(root_url){
    var t;
    if (!root_url || !(t = this.get_trial_active(root_url, true)))
        return;
    var site;
...
TrialLocal.prototype.is_trial_expired = function(root_url){
    return this.trials.some(function(t){
        return t.domain.includes(root_url) && t.expire_ts<=Date.now(); });
};

TrialLocal.prototype.is_trial_grace_period = function(root_url){
    var now = Date.now(), trial = this.get_trial_active(root_url, true);
    return trial && trial.grace_ms && now>trial.expire_ts &&
        now<trial.grace_ms+trial.expire_ts;
};

TrialLocal.prototype.get_trial_active = function(root_url, ignore_ts){
...
TrialLocal.prototype.get_trial_using = function(root_url){
    var trial;
    if (!(trial = this.get_trial_active(root_url)))
        return;
...
TrialLocal.prototype.get_trial_agent_port = function(rule, def_port){
    return def_port; };

TrialLocal.prototype.uninit = function(){
    if (this.trial_monitor)
...
TrialRemote.prototype.set_time_left = function(root_url, value){
    ...
    return etask('trial_set_time_left', [function(){
        be_lib.perr_ok({id: 'set_time_left', info: {root_url: root_url,
            value: value}});
        return ajax.json({url: conf.url_ccgi+'/trial_set_time_left',
...
function get_trials(url){
    ...
    return etask('get_trials', [function(){
        ...
        return ajax.json({url: conf.url_ccgi+url, qs: auth});
    }, function(trials){
...
E.get_trial_agent_port = function(rule, def_port){
    return zutil.is_mocha() ? def_port :
...
E.get_next_trial_ts = function(root_url){
    ...
    return trial.expire_ts+(site.trial.wait||2*date.ms.MIN);
};
...
function update_config(){
    var is_remote = be_ext.get('gen.remote_trial');
    if (handler && (is_remote && handler.name=='remote' ||
        !is_remote && handler.name=='local'))
    {
        return;
    }
    if (handler)
        handler.uninit();
    handler = is_remote ? new TrialRemote() : new TrialLocal();
    refresh_trials();
}
