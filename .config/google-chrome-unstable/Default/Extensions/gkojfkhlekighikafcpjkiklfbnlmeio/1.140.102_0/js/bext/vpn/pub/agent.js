...
define(['/bext/pub/browser.js', '/bext/pub/backbone.js', 'underscore',
    ...
    '/bext/pub/version_util.js', '/bext/pub/util.js'],
    function(B, be_backbone, _, etask, zerr, ajax, be_lib, be_ext, storage,
    date, zutil, be_info, unblocker_lib, svc_util, be_version_util, be_util){
...
E.is_trial_rule = function(rule){
    ...
    return E.be_rule && E.be_rule.is_trial_rule(rule) &&
        (min_ver = be_ext.get('gen.trial_agents_min_ver')) &&
        be_version_util.cmp(be_util.version(), min_ver)>=0;
};

E.is_pool_rule = function(rule){
    var min_ver, site_conf;
    return be_ext.get('is_premium') && rule && E.be_premium &&
        (min_ver = be_ext.get('gen.pool_agents_min_ver')) &&
        be_version_util.cmp(be_util.version(), min_ver)>=0 &&
...
var set_verified_agents = function(ret, rule){
    ...
    for (var s in routes)
    {
        ...
        {
            rule_agents[t!='pool' ? t : agent.pool ? t : 'def'][s] =
...
E.resolve_agents = function(r, exclude, opt){
    ...
    return etask('resolve_agents', [function(){
