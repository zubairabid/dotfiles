...
define(['/util/etask.js', '/util/url.js', '/util/util.js',
    ...
    function(etask, zurl, zutil, be_version_util){
var E = {debug: {}};
...
function init_rules_from_storage(){
    ...
    if (!o || !o.ver || !o.rules ||
        be_version_util.cmp(o.ver, min_rules_ver)<0)
    {
        rules = [];
    }
    else
...
function is_feature_enabled(feature, extra){
    ...
    return m.every(function(f){
        ...
        return !CGM(d_path) && (!CGM(v_path)
            || be_version_util.cmp(api.version, CGM(v_path))>=0)
            && (!extra || CGM(e_path));
    });
}
...
function send_mitm_perr(url, info, hc){
    ...
    if (!zutil.is_mocha())
    {
        perr(assign({filehead: rnd_id<CGM('send_dbg_log') ?
...
E.is_popup_needed = function(tab_url, tab_id){
    ...
    var ret = E.inited && E._is_mitm_active(tab_id) && api
        ...
        be_version_util.cmp(api.version, o.min_version)>=0);
...
E.check_mitm_blocking = function(opt){
    ...
    var hc = {host: root_url, tab_url: tab_url, url: opt.url,
        tab_id: tab_id, info: {}, test: opt.hook=='test',
        redir: opt.redir};
    ...
    var test_url = !prev_code && !opt.geo_rule ? proto=='http'
        ? url.replace('http', 'https') : url : 'https://'+get_domain_url(url)
...
E.is_mitm_allowed = function(tab_id, tab_url, url, details, geo_rule){
    var has_geo = CGM('detect_geo_rules') && geo_rule;
    return E.inited && (has_geo || !geo_rule && !is_valid_resp(details))
...
E.init = function(_api){
    ...
    if (CGM('disable') || !CGM_if_includes('country_whitelist', local.country)
        ...
        || CGM('min_version') && be_version_util.cmp(api.version,
        CGM('min_version'))<0)
    {
        return;
    }
