...
Protect_ui.prototype.set_enabled = function(enable, host, hosts, is_mitm){
    if (!enable)
    {
        if (this.is_enabled_for_root_url({ignore_all_browser: true}))
            this.set_enabled_for_root_url(false, host, hosts, is_mitm);
        if (this.is_enabled_for_browser())
            this.set_enabled_for_browser(false);
        return;
    }
    return this.set_enabled_for_root_url(true, host, hosts, is_mitm);
};
Protect_ui.prototype.set_enabled_for_root_url = function(enable, host, hosts,
    is_mitm)
{
    return etask([function(){
...
E.set_active_url = function(root_url, country, disable){
    ...
    var rule_enabled;
    zerr.debug('tab:%d set_active_url %s %s', get_tab_id(), root_url, country);
    return etask([function(){
...
var unblock_protect_view_class = Backbone.View.extend({
    ...
    initialize: function(){
        this.needs_plus_view = new modal_view_class({
            ...
            text_yes: 'Get PLUS',
            on_yes_click: this._on_get_plus_click,
    ...
    unblock_on_click: function(e){
        ...
        set_unblock(e.value, {host: host, hosts: hosts});
    },
    protect_on_click: function(e){
        ...
        E.protect_ui.set_enabled(e.value, host, hosts, false);
    },
    render: function(){
        ...
        ui_lib.render_main_switch(this.$a[0],
            {value: !!is_unblock,
            onClick: this.unblock_on_click.bind(this),
...
E.script_set = function(rule, val){
    var ts = Date.now(), tabid = get_tab_id();
    return etask([function(){
        clr_working();
        ...
        if (val.enabled && site_conf && site_conf.require_plus &&
            !E.be_ext.get('is_premium') && site_conf.trial && !trial)
        {
